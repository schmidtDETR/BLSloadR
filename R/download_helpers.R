...
create_bls_object <- function(
  ...
  summary_info <- list(
    ...
    files_with_issues = sum(sapply(download_diagnostics, function(x) {
      !is.null(x) && length(x$warnings) > 0
    })),
...
smart_bls_download <- function(url, cache_dir = NULL, verbose = FALSE) {
  ...
  if (httr::status_code(response_head) != 200) {
    if (file.exists(local_path)) {
      ...
      stop(
        "Could not connect to BLS server and no local cache exists. Status: ",
  ...
  if (needs_download) {
    if (verbose) {
      message(paste0(
        "For ",
        url,
        ": Cached file missing or outdated: Downloading new file."
      ))
...
print_bls_warnings <- function(bls_obj, detailed = FALSE, silent = FALSE) {
  ...
  if (length(warnings) == 0) {
    if (!silent) {
      message("No warnings for", summary$data_type, "data download\n")
    }
    return(invisible(character(0)))
  }
  ...
  if (!silent) {
    ...
    if (!is.null(summary$final_dimensions)) {
      message(
        ...
        paste(summary$final_dimensions, collapse = " x "),
        "\n"
      )
    }
    message("\n")
    ...
    if (detailed && length(diagnostics) > 0) {
      ...
      for (file_name in names(diagnostics)) {
        ...
        if (!is.null(diag) && length(diag$warnings) > 0) {
          ...
          message(
            "  Original dimensions:",
            paste(diag$original_dimensions, collapse = " x "),
            "\n"
          )
          message(
            "  Final dimensions:",
            paste(diag$final_dimensions, collapse = " x "),
            "\n"
          )
          message("  Issues:\n")
