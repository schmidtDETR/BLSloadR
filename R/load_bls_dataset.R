...
load_bls_dataset <- function(
  ...
  if (!is.null(which_data)) {
    ...
    if (!is.null(pattern)) {
      ...
      if (length(matches) > 0) {
        ...
        message(
          "Warning: No file matching '",
          pattern,
          "' found. Falling back to manual selection/default.\n"
        )
      }
    }
  }
  ...
  if (!is.null(selected_aspect_file)) {
    aspect_url <- setNames(
      paste0(base_url, selected_aspect_file),
      selected_aspect_file
    )
    ...
    tryCatch(
      {
        ...
        if (length(available_join_cols) > 0) {
          ...
          if (!suppress_warnings) {
            message(
              "Aspect file joined successfully on: ",
              paste(available_join_cols, collapse = ", ")
            )
          }
        } else {
          if (!suppress_warnings) {
            message(
              "Warning: Could not join aspect file - no matching columns found"
            )
          }
        }
      },
      ...
      tryCatch(
        {
          ...
          if (ncol(map_dt) == 2) {
            ...
            if (join_col %in% names(full_dt)) {
              ...
              processing_steps <- c(
                processing_steps,
                paste0("joined_mapping_", gsub("\\.", "_", map_file))
              )
            } else {
              if (!suppress_warnings) {
                message(
                  "Skipping mapping file ",
                  map_file,
                  " - join column '",
                  join_col,
                  "' not found in data"
  ...
  if (return_full) {
    return(list(
      ...
      data = data_dt,
      series = series_dt,
      aspect = if (!is.null(selected_aspect_file)) {
        get_bls_data(downloads[[selected_aspect_file]])
      } else {
        NULL
      },
