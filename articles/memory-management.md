...
BLSloadR handles large datasets from the Bureau of Labor Statistics,
some exceeding 500MB in compressed format. This vignette explains how
the package manages memory efficiently and how users can avoid crashes
...
Instead of reading all columns as character strings (memory-intensive),
the package now:
...
**You don’t need to do anything** - this happens automatically when you
download data.
...
- Removes intermediate objects with
  [`rm()`](https://rdrr.io/r/base/rm.html)
- Triggers garbage collection with
...
Instead of downloading the full dataset and filtering in R, use built-in
filters:
...
If you’re developing or running tests in a memory-constrained
environment:
...
1.  **Close other applications** to free RAM before downloading large
    files
...
3.  **Use specific series IDs** instead of characteristic filters when
    possible:
...
**Note:** “In-Memory (Optimized)” reflects the improvements in BLSloadR
0.3.0+.

## Best Practices

1.  **Start small**: Test your code with specific states or series
    before downloading full datasets

2.  **Use persistent caching**: Set up a permanent cache directory to
    avoid re-downloading

3.  **Monitor memory**: Use `pryr::mem_used()` or Task Manager to track
    memory usage

4.  **Close and restart R**: Between large operations to ensure clean
    memory state

5.  **Leverage subset caching**: Functions like
    [`get_cps_subset()`](https://schmidtdetr.github.io/BLSloadR/reference/get_cps_subset.md)
    cache filtered results, making subsequent queries fast
...
4.  Open an issue at <https://github.com/schmidtDETR/BLSloadR/issues>
    with:
    - Your system RAM
    - The specific function call that’s failing
    - Any error messages

The BLSloadR package is designed to handle large files efficiently, but
memory constraints are ultimately limited by your system’s available
RAM.
